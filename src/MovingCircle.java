...
/* This class extends from the MovingObject class and creates an instance of the MovingObject which is a MovingCircle.  The class contains
 ...
 * a path transition for the circle using the randomly generated entry and exit points.
 */
...
public class MovingCircle extends MovingObject {
	...
	/** Randomizer-line path:  Creates a random line path from the top to the bottom of the screen for animation of the MovingCircle object */
	@Override
	public void playMovingObject_Top_Bottom(MovingObject movingObject, int paneWidth, int paneHeight) {

		// Create points to use for line that will be transition path for circle.
		// Circle will come in from the top of the window, random location.
		setX1((int)(Math.random()*(paneWidth+1)));
		setY1(0);
		setX2((int)(Math.random()*(paneWidth+1)));
		setY2(paneHeight+50);

		// Create a path transition for the circle to move across the screen
		PathTransition topBottomObject_pt = new PathTransition();
		topBottomObject_pt.setDuration(Duration.millis(6667)); // It takes a circle 6.667 seconds to pass along the full length of the screen
		topBottomObject_pt.setPath(new Line(getX1(), getY1(), getX2(), getY2()));  //  Path is the line through these two sets of points
		topBottomObject_pt.setNode(movingObject);   // Node that follows path is the circle
		topBottomObject_pt.setCycleCount(3);   // cycle 3 times  (3  x 6.667 seconds = 20 seconds)
		topBottomObject_pt.play();  // Start animation
	}

	/** Randomizer-line path:  Creates a random line path from the bottom to the top of the screen for animation of the MovingCircle object */
	@Override
	public void playMovingObject_Bottom_Top(MovingObject movingObject, int paneWidth, int paneHeight) {

		// Create points to use for line that will be transition path for circle.
		// Circle will come in from the bottom of the window, random location.
		setX1((int)(Math.random()*(paneWidth+1)));
		setY1(paneHeight);
		setX2((int)(Math.random()*(paneWidth+1)));
		setY2(-50);
		...
		bottomTopObject_pt.setDuration(Duration.millis(6667)); // It takes a circle 6.667 seconds to pass along the full length of the screen
		bottomTopObject_pt.setPath(new Line(getX1(), getY1(), getX2(), getY2()));  //  Path is the line through these two sets of points
		bottomTopObject_pt.setNode(movingObject);   // Node that follows path is the circle
		bottomTopObject_pt.setCycleCount(3);   // cycle 3 times  (3  x 6.667 seconds = 20 seconds)
	...
	public void playMovingObject_Right_Left(MovingObject movingObject, int paneWidth, int paneHeight) {
		...
		// Create points to use for line that will be transition path for circle.
		// Circle comes in from the right side edge, random location.
		setX1(paneWidth  +10);
		...
		rightLeftObject_pt.setDuration(Duration.millis(6667));   // It takes a circle 6.667 seconds to pass along the full length of the screen
		rightLeftObject_pt.setPath(new Line(getX1(), getY1(), getX2(), getY2()));  // Path is the line through these two sets of points
		rightLeftObject_pt.setNode(movingObject);   // Node that follows path is the circle
		rightLeftObject_pt.setCycleCount(3);   // cycle 3 times  (3  x 6.667 seconds = 20 seconds)
	...
	public void playMovingObject_Left_Right(MovingObject movingObject, int paneWidth, int paneHeight) {
		...
		// Create points to use for line that will be transition path for circle.
		// Circle will come in from the left side of the window, random location.
		setX1(0);
		setY1((int)(Math.random()*(paneHeight+1)));
		setX2(paneWidth + 100);
		setY2((int)(Math.random()*(paneHeight+1)));
		...
		leftRightObject_pt.setDuration(Duration.millis(2857));   // It takes a circle 6.667 seconds to pass along the full length of the screen
		leftRightObject_pt.setPath(new Line(getX1(), getY1(), getX2(), getY2()));  //  Path is the line through these two sets of points
		leftRightObject_pt.setNode(movingObject);   // Node that follows path is the circle
		leftRightObject_pt.setCycleCount(7);   // cycle 3 times  (3  x 6.667 seconds = 20 seconds)
